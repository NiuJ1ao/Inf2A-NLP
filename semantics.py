...
def sem(tr):
    ...
    # add code here
        ...
        return '(\\x.(x = ' + sem(tr[0]) + '))'   
    elif (rule == 'Rel -> WHO VP'): # add more code here
        return sem(tr[1])
    elif (rule == 'Rel -> NP T'):
        ...
        return '(\\x.exist y.(' + sem(tr[1]) + '(y) & ' + sem(tr[0]) + '(y)(x)))'
    elif (rule == 'VP -> BE A'):
        return sem(tr[1])
    elif (rule == 'VP -> BE NP'):
        return sem(tr[1])
    elif (rule == 'VP -> VP AND VP'):
        return '(' + sem(tr[1]) + '(' + sem(tr[0]) + '))' + '(' + sem(tr[2]) + ')'
    elif (rule == 'QP -> VP'):
        return sem(tr[0])
...
# A = lp.parse('(\\x.exist y.((\\x.((\\x.N_duck(x))(x) & (\\x.(exists y.(\\x.(x = Mike))(y) &(\\y.(\\x.T_like(x,y)))(y)))(x)))(y) & (\\y.(\\x.T_like(x,y)))(y)(x)))')
